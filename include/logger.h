...
template <bool value, typename TrueResult, typename FalseResult> struct Select {
...
template <typename T, int Option> struct ValueSelect {
  typedef T Type;
...
template <> struct ValueSelect<std::ostream, 0> {
...
template <LogLevel CurrentLevel> struct Logger {
